...
/**
 * Get recently modified meetings and store attendee information
 */
const processMeetings = async (domain, hubId, q) => {
  ...
  while (hasMore) {
    const lastModifiedDate = offsetObject.lastModifiedDate || lastPulledDate;
    const lastModifiedDateFilter = generateLastModifiedDateFilter(lastModifiedDate, now, 'hubspot_owner_id');
    ...
    for (const meeting of data) {
      ...
      // Retrieve attendees for the meeting
...
const fetchMeetingAttendees = async (meetingId) => {
  try {
    const response = await hubspotClient.crm.engagements.v1.engagements.associations.get({
